...
- **Brand drug names are curated fictional names**, drawn from a fixed list of 245 (`BRAND_DRUG_NAMES`) rather than assembled at random. Each was built from invented morphemes and then screened, on **2026-08-16**, against: WHO INN class stems; an append-only denylist of real products (including two FDA veterinary products that an earlier, human-brands-only screen missed); a list of offensive substrings; and every drug name this package itself ships. The list was then read name by name — which is only possible because it is 245 names and not the 31,500 the morphemes can produce, and is why it is a list at all.
  ...
  The zh_CN Chinese names (`ZH_BRAND_NAMES`) go through the same screens plus one of their own — the characters that read as a generic drug rather than a brand (`维` = vitamin, `定` as in 安定/diazepam) — and then a **Simplified-Chinese reading pass**, asking of each pair: is it a real company or product, an ordinary word, a personal name, a place; does it read as a substance; does it read as a plausible invented brand at all. Two passes have run: **2026-08-16** read 64 candidates and rejected 58, and **2026-08-17** widened the character pool by eleven characters, read 370 more, rejected 337, and withdrew one of the six names the first pass had shipped (`复安`, the tail of 胃复安, the household name for metoclopramide). **25 names ship**, so a zh_CN `brand_drug()` draws 25 Chinese halves rather than 6.

  Twenty-five is the honest yield, not a target: the second pass read 370 candidates to add 20. Chinese pharmaceutical brands recycle `康`/`泰`/`瑞`/`舒`/`益`/`欣` across so many marketed products and companies that the pairs which sound most like a real brand are the likeliest to be one, and every rejection is recorded with its reason in `scripts/generate_brand_names.py`. Both passes were **an LLM reading pass — not a trademark search and not a native speaker's sign-off**; neither is claimed, and reports of a collision are welcome.
